      ...
      <div className="w-full pl-4 border-l-2 border-[var(--color-accent)] space-y-3">
        ...
        <div className="text-sm text-[var(--color-ink)] space-y-2">
          {message.content === 'Thinking...' ? (
            <div className="flex items-center gap-2 text-xs text-[var(--color-accent)] font-mono animate-pulse py-1">
              <Sparkles className="w-3.5 h-3.5 animate-spin text-[var(--color-accent)] shrink-0" />
              <span>Deliberating turn response...</span>
            </div>
          ) : cleanedContent.trim() ? (
            renderFormattedText(cleanedContent)
          ) : (
            <p className="text-xs font-mono text-[var(--color-ink-muted)] italic py-1">
              ⚠️ No content generated by model turn.
            </p>
          )}
        </div>
